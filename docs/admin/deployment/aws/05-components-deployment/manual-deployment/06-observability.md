...
- `--values kibana/values-aws.yaml` - Uses AWS-specific configuration
...
- `https://kibana.codemie.example.com` (custom domain)
...
```bash
bash ./kibana-dashboards/manage-kibana-dashboards.sh \
  --url "https://kibana.codemie.example.com" \
  --k8s-auth \
  --non-interactive
```
...
curl -k https://kibana.<your-domain>
```
