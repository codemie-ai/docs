...
The domain configuration should already be set from the Getting Started section. Verify the values in <code>codemie-api/values-{props.cloudName}.yaml</code> are correct:
...
{`helm upgrade --install codemie-api \\
  ...
  --version x.y.z \\
  --namespace codemie \\
  -f ./codemie-api/values-${props.cloudName}.yaml \\
  --wait \\
  --timeout 600s`}
</CodeBlock>
...
- <code>-f ./codemie-api/values-{props.cloudName}.yaml</code> - Uses {props.cloudProvider}-specific configuration
