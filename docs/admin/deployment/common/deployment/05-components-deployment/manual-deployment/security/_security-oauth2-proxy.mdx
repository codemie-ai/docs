...
### Step {props.installStepNumber}: Install OAuth2 Proxy Helm Chart
...
{`helm upgrade --install oauth2-proxy oauth2-proxy/. \\
  -n oauth2-proxy \\
  --values oauth2-proxy/values-${props.cloudName}.yaml \\
  --wait \\
  --timeout 900s \\
  --dependency-update`}
</CodeBlock>
...
- <code>--values oauth2-proxy/values-{props.cloudName}.yaml</code> - Uses {props.cloudProvider}-specific configuration

### Step {props.verifyStepNumber}: Verify OAuth2 Proxy Deployment
