...
The domain configuration should already be set from the Getting Started section. Verify the values in `codemie-api/values-{props.cloudName}.yaml` are correct:
...
- `-f ./codemie-api/values-{props.cloudName}.yaml` - Uses {props.cloudProvider}-specific configuration
