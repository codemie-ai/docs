...
{`helm upgrade --install elastic elasticsearch/. \\
  -n elastic \\
  --values elasticsearch/values-${props.cloudName}.yaml \\
  --wait \\
  --timeout 900s \\
  --dependency-update`}
</CodeBlock>
...
- <code>--values elasticsearch/values-{props.cloudName}.yaml</code> - Uses {props.cloudProvider}-specific configuration
