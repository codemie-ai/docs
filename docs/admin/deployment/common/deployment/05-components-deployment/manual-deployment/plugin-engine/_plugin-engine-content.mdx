...
- <code>--values ./codemie-nats/{props.valuesFileName}</code> - Uses {props.cloudName}-specific configuration
...
{props.authStepContent}

### Step {props.installStepNumber}: Install NATS Auth Callout Helm Chart
...
- `--namespace codemie` - Deploys to codemie namespace
- <code>-f ./codemie-nats-auth-callout/{props.valuesFileName}</code> - Uses {props.cloudName}-specific configuration

:::tip Version Number
Use the same version number you retrieved in the <a href={props.gettingStartedLink}>Getting Started</a> section.
:::

### Step {props.verifyStepNumber}: Verify NATS Auth Callout Deployment
