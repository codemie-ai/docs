...
- `--values ./codemie-nats/{props.valuesFileName}` - Uses {props.cloudName}-specific configuration
...
- `--namespace codemie` - Deploys to codemie namespace
- `-f ./codemie-nats-auth-callout/{props.valuesFileName}` - Uses {props.cloudName}-specific configuration
