...
CodeMie Lightweight runs on a single EC2 instance with supporting AWS services. Terraform provisions the following resources depending on the network mode:
...
<Tabs>
  <TabItem value="ip" label="IP Mode (default)" default>
    ...
    ![IP Mode Diagram](./images/ip-mode-diagram.drawio.svg)
  ...
  <TabItem value="domain" label="Domain Mode (ALB + ACM)">
    ...
    ![Domain Mode Diagram](./images/domain-mode-diagram.drawio.svg)

  </TabItem>
  <TabItem value="private" label="Private IP Mode">
    When `TF_VAR_private_ip_only=true`, EC2 is placed in a private subnet with NAT Gateway for outbound traffic. Access is via VPN only:

    ![Private Mode Diagram](./images/private-mode-diagram.drawio.svg)
...
![Docker Compose Services](./images/docker-compose-diagram.drawio.svg)
...
- [Deployment](/admin/deployment/lightweight/deployment) — Deploy CodeMie Lightweight with Terraform
- [BYO EC2](/admin/deployment/lightweight/byo) — Deploy on an existing EC2 instance without Terraform
