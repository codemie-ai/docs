...
| `CODE_EXECUTOR_RUNTIME_CLASS_NAME`        | string  | `"gvisor"`                        | Kubernetes `runtimeClassName` applied to Job pods when `CODE_EXECUTOR_SANDBOX_MODE=sandbox-jobs`. Must match an installed runtime class (`gvisor` or `kata-containers`).                                                                                          |
...
| `CODE_EXECUTOR_MAX_THREADS`               | integer | `64`                              | Maximum number of threads allowed per execution, enforced inside the sandbox process (both shared and jobs modes). Limits thread-based resource exhaustion attacks such as billion laughs.                                                                        |
