...
**Tolerations:** `CODE_EXECUTOR_TOLERATIONS` is a scheduling control, not an isolation control — it lets executor Job pods land on tainted nodes (e.g. a dedicated, tainted node pool). It does not affect sandbox isolation on its own, but review the trust level and workloads on any node pool a toleration opens up before enabling it, especially if that pool has different security guarantees than the rest of the cluster.

Example: to dedicate a kata-containers node pool to code execution, taint the pool with `runtime=kata-containers:NoSchedule`, set `CODE_EXECUTOR_RUNTIME_CLASS_NAME=kata-containers` to match the pool's installed runtime class, and set `CODE_EXECUTOR_TOLERATIONS='[{"key":"runtime","operator":"Equal","value":"kata-containers","effect":"NoSchedule"}]'` (via the Helm chart's `customEnv` value — see `deploy-templates/values.yaml`) so Job pods can schedule onto it.
