...
| `mcpCustomServersDisabled`                | Assistant/Workflow config → MCP Servers                   | Catalog-only MCP mode enforced at save and runtime                 | Catalog-only MCP mode               | When enabled, only admin-managed catalog of MCP servers can be used |
