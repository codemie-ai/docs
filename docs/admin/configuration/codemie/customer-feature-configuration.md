...
components:
  ...
  #       identifier, falling back to `fallbackLabel` when no rule matches.
  - id: "features:cliAnalytics"
    settings:
      enabled: false
      name: "CLI Analytics"
      description: "CLI Analytics tab and OTEL proxy: per-session coding-agent cost, tooling, repository and efficiency analytics"
      availableForExternal: false
      deliveryFramework:
        rules:
          - regex: "^sdlc-factory:"
...
Customer-level toggle gating whether tool-call confirmation is enforced, plus a customer-wide floor on how strict that enforcement must be.

**Where it appears:** Assistant configuration → Tools section (policy dropdown: Manual / Guarded / Auto, plus an "Allow override" toggle). When override is allowed on the assistant, the same policy dropdown also appears in the chat prompt.

**How it works:**

- **When `enabled: false`** - all tool calls auto-approve, regardless of any assistant, request, or conversation-level policy.
- **When `enabled: true`** - the effective policy resolves in this order: request override → conversation policy → assistant config. If the assistant sets `allow_override: false`, request and conversation overrides are ignored and only the assistant config applies.
- `min_tool_call_policy` sets a customer-wide floor - the resolved policy is clamped to be at least as strict as this value (`manual` > `guarded` > `auto`).

**Fields used in this section:**

```yaml
settings:
  enabled: true                   # Required — enables tool-call confirmation enforcement
  min_tool_call_policy: "guarded" # Optional — floor policy (manual | guarded | auto); resolved policy is clamped to at least this strict
```
...
```yaml
components:
  # WHERE: Assistant configuration → Tools section (policy dropdown + "Allow override" toggle);
  #        same dropdown appears in chat prompt when override is allowed
  # ENABLED: Effective policy resolves from request override -> conversation policy -> assistant config
  #          (assistant config only if allow_override is false); min_tool_call_policy clamps the result
  # DISABLED: All tool calls auto-approve regardless of policy
  # NOTE: min_tool_call_policy is a customer-wide floor (manual | guarded | auto)
  - id: "tool_permissions"
    settings:
      enabled: true
