...
This document provides a comprehensive reference for all configuration parameters available in the CodeMie API. These parameters control application behavior, AI provider integrations, storage, security, and more. Configure them through environment variables or `.env` files.
...
| `LOG_LEVEL` | string | `"INFO"` | Minimum log severity to output; use `DEBUG` for troubleshooting, `WARNING` for production |
...
For GPT-4, GPT-3.5, and embedding models via Azure OpenAI Service.
...
For Claude 3 Opus, Sonnet, and Haiku models via Anthropic's native API.
...
For PaLM 2, Gemini, and Claude via Google Cloud's Vertex AI platform.
...
| `MERMAID_USE_MERMAID_INC` | boolean | `false` | Use hosted Mermaid Inc. service instead of local renderer (requires internet) |
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `FILES_STORAGE_TYPE` | string | `"filesystem"` | Storage backend: `filesystem` (local), `aws` (S3), `azure` (Blob), `gcp` (Cloud Storage) |
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `ENCRYPTION_TYPE` | string | `"plain"` | Encryption method: `plain` (none), `aws` (KMS), `azure` (Key Vault), `gcp` (Cloud KMS), `vault` (HashiCorp) |
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `IDP_PROVIDER` | string | `"local"` | Identity provider: `keycloak` (enterprise SSO), `local` (built-in), `oidc` (generic OIDC) |
...
Connect CodeMie to external services for enhanced capabilities.
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `GOOGLE_SEARCH_API_KEY` | string | `""` | API key for Google Custom Search integration |
| `GOOGLE_SEARCH_CSE_ID` | string | `""` | Custom Search Engine ID for scoped web searches |
...
Configure AI tool selection, code analysis integrations, and tool execution limits.
...
Tool-specific environment variables for configuring individual CodeMie tool behaviors. These parameters control execution environments, security policies, and feature access for built-in tools.
...
| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `CODE_EXECUTOR_EXECUTION_MODE` | string | `"local"` | Execution mode: `sandbox` (isolated K8s pod), `local` (embedded kernel, less secure) |
| `CODE_EXECUTOR_KUBECONFIG_PATH` | string | `""` | Path to kubeconfig for Kubernetes authentication (optional, uses in-cluster config if empty) |
...
Control access to file system manipulation tools. These tools provide direct filesystem access and are restricted to administrators.

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `FILE_SYSTEM_TOOLS_ENABLED` | boolean | `false` | Enable file system tools (read/write/command-line) for admin users only |

:::danger Security Warning
**⚠️ Critical Security Setting**

Setting `FILE_SYSTEM_TOOLS_ENABLED=true` grants direct file system access to administrators, including:
- Reading any file on the server
- Writing/modifying files
- Executing shell commands

**Only enable in:**
- Trusted, internal environments
- When proper access controls are in place
- With comprehensive audit logging enabled

**Never enable in:**
- Multi-tenant environments
- Public-facing deployments
- Environments with untrusted administrators
:::
