...
When creating a new Jira issue from an existing one, the assistant can copy attachments across tickets on request. Both attachment transfer and image text extraction are **opt-in** — the tool never copies files or runs image recognition unless explicitly asked.
...
To copy all attachments from an existing ticket into a newly created one, include the request in the prompt:

> "Create a bug in project BUG from PROJ-123 and copy its attachments"

The assistant creates the new issue and transfers all files (screenshots, logs, documents) from the source ticket. The files are identical to the originals.

### Extract text from image attachments (OCR)

To extract text from image attachments, request it explicitly:

> "Create a bug in project BUG from PROJ-123, copy its attachments, and extract text from any images"

The assistant runs image recognition on every image attachment and posts a single comment on the new issue containing the extracted content. This is a separate step from attachment copy — either option can be requested independently.
...
:::warning
Image text extraction uses a vision model and incurs additional processing cost. Only request it when the extracted content is needed.
:::
