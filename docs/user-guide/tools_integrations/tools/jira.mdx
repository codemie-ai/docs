...
Two authentication methods are available for the Jira integration:

- **Personal Access Token (PAT)** — a Jira API token is created manually and pasted into the integration form. This is the classic approach and works for both Jira Cloud and Jira Self-hosted.
- **OAuth 2.0 (Atlassian 3LO)** — an administrator configures the Atlassian OAuth application once (shared `client_id` and `client_secret`), and each member authorizes under their own Atlassian account. Tokens are per-user: calls run as the member who connected, so no one borrows another member's token. **This option is only available for Jira Cloud and requires the platform's Jira OAuth feature to be enabled by an administrator.**

:::info OAuth availability
The OAuth 2.0 sign-in option is shown in the integration form only when the platform administrator has enabled it. If the toggle is not visible, the `JIRA_OAUTH_ENABLED` flag is off. Contact the platform administrator to enable it. See the [OAuth Integration Setup](../../../admin/configuration/codemie/api-configuration.md#jira-oauth) admin guide.
:::
...
:::
  ...
  <TabItem value="old-ui" label="Jira Self-hosted">
...
2.3. Set **Credential Type** to **Jira** and fill in the **Project Name**, **Global Integration** toggle, and **Alias** fields. Then choose an authentication method:

<Tabs groupId="jira-auth">
...
- **Is Jira Cloud**:
  - ✅ **Check this box** if using **Jira Cloud**
  - ❌ **Leave unchecked** if using **Jira Self-hosted**
- **Username/email for Jira**: The Atlassian account email (e.g., `user@example.com`).
...
Click **Test** (optional) to verify the connection, then click **Save**.

  </TabItem>
  <TabItem value="oauth" label="OAuth 2.0 (Atlassian)">

:::info Prerequisite
The OAuth 2.0 option is only available when the platform administrator has enabled it (the `JIRA_OAUTH_ENABLED` flag). If the **Use OAuth 2.0 sign-in** toggle is not shown, contact the platform administrator.
:::

OAuth 2.0 integrations are **per-user**: the OAuth application credentials (`Client ID`, `Client Secret`, `Callback Base URL`) are entered in the integration form when creating the integration, and each member then connects under their own Atlassian account. Calls to Jira run as the individual member — no shared token.

**Steps:**

2.3.1. Enable the **Use OAuth 2.0 sign-in** toggle in the integration form. The form shows the OAuth application fields: **Client ID**, **Client Secret**, and **Callback Base URL**. Fill these in with the values from the Atlassian OAuth application registered for this platform.

2.3.2. Click **Sign in with Jira**. A browser popup opens the Atlassian authorization page.

2.3.3. Log in to Atlassian and grant the requested permissions. The popup closes automatically on success.

2.3.4. The form shows a confirmation that the account is connected. The Atlassian `cloud_id` is resolved automatically — no URL field is required.

2.3.5. Click **Save** to create the integration.
...
:::tip
Both User-level and Project-level integrations are supported. User-level integrations are available across all projects; Project-level integrations are specific to a single project.
:::

## 2a. Connect from Chat (OAuth only)

When a run first uses a Jira integration that a member has not yet connected, the chat displays a **"Connect your Jira account"** prompt with a **Sign in** button.
